...
public class PinSpawner : MonoBehaviour { // 페그 자동 생성기
    ...
    public Transform myPeg; // 씬에 배치한 기본 페그
    ...
    void spawnPegs() {
        if (myPeg == null) {
            Debug.LogError("기본 페그가 설정되지 않았습니다.");
            return;
        }

        float startY = myPeg.position.y; // 기본 페그의 y좌표
        float totalWidth = (cols - 1) * spacing; // 전체 너비
        float startX = -totalWidth / 2; // x좌표 중심 정렬
        float firstRowY = startY; // 첫 번째 페그의 y좌표 = 기본 페그의 y좌표
